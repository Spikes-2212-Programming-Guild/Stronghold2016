...
public class FeedForwardStraight extends Command {
	...
	public FeedForwardStraight(double distance) {
		...
		this.feedForward = new FeedForward(Constants.KP,
				Constants.MAX_LEFT_VELOCITY, Constants.MAX_ACCELERATION,
				Constants.MAX_DECCELERATION, this.distance);
	}

	// Called just before this Command runs the first time
	protected void initialize() {
		time = System.currentTimeMillis();
		setTimeout(this.feedForward.getTotalTime() + 1);
	}
	...
	protected void execute() {
		...
		Robot.drivetrain.forward(feedForward.getVoltage(expected[1],
				expected[2], expected[0],
				Math.abs(Robot.drivetrain.getLeftDistance())));
